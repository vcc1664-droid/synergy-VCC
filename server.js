  ...
  try {
    ...
    if (!notionApiKey || !rawDbId) {
      return res.status(500).json({
        success: false,
        error: 'Server configuration error: NOTION_API_KEY or NOTION_DATABASE_ID is missing in server environment.',
      })
    }

    const notionDatabaseId = rawDbId.replace(/-/g, '')

    // Step 1: Fetch Database schema to detect exact column names & property types
